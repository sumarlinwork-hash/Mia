...
export default function Creator() {
  ...
  const { hue } = useTheme();
  const [cardOpacity, setCardOpacity] = useState<number>(0.72);
  ...
  return (
    <div className="min-h-screen p-8 text-white">
      <div className="max-w-7xl mx-auto space-y-10">
        <section className="rounded-[2.5rem] border border-white/10 bg-white/[0.04] shadow-2xl shadow-black/20 backdrop-blur-3xl p-8">
        ...
        <section className="grid gap-8 xl:grid-cols-[1.25fr_0.85fr]">
          <div className="space-y-6">
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 shadow-2xl shadow-black/20 backdrop-blur-3xl">
              <div className="flex items-center justify-between gap-4 mb-8">
                ...
                <button className="inline-flex items-center gap-2 rounded-2xl bg-primary px-5 py-3 text-black text-xs font-bold uppercase tracking-[0.24em] transition-all hover:scale-[1.01]">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {[1, 2, 3, 4].map((index) => (
                  <div
                    key={index}
                    className="rounded-[2rem] border border-white/10 overflow-hidden shadow-lg"
                    style={{
                  ...
                  >
                    <div className="relative h-56 overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black" style={{ opacity: 1 - cardOpacity }} />
                    </div>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 shadow-2xl shadow-black/20 backdrop-blur-3xl">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <h3 className="text-xl font-bold">Creator Preview Settings</h3>
                  <p className="text-sm text-white/50 mt-2">This Creator preview respects the global appearance theme from Companion settings.</p>
                </div>
                <div className="w-full max-w-sm">
                  <label className="text-xs uppercase tracking-[0.28em] text-white/40">Preview opacity</label>
                  <input
                    type="range"
                    min={0.3}
                    max={1}
                  ...
                  />
                  <div className="mt-2 text-xs text-white/50">{Math.round(cardOpacity * 100)}%</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <aside className="space-y-6">
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-8 shadow-2xl shadow-black/20 backdrop-blur-3xl">
              ...
              <div className="mt-5 flex items-center gap-3">
                <button className="inline-flex items-center gap-2 rounded-3xl bg-primary px-5 py-3 text-black font-bold uppercase tracking-[0.24em] transition-all hover:shadow-[0_0_30px_rgba(0,255,204,0.2)]">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-6 text-sm text-white/70">
